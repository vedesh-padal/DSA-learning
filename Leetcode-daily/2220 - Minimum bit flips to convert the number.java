/*
  ...
  (including any leading zeros not shown) and flip it. We can flip the first bit from 
...
class Solution {
  ...
  private static int countSetBits(int num) {
    int count = 0;
    while (num != 0) {
      count += num & 1; // if the last bit is 1, we add it to the count
  ...
  public int minBitFlips(int start, int goal) {
    int xor = start ^ goal; // perform xor to get the binary form with only differing bits as set
