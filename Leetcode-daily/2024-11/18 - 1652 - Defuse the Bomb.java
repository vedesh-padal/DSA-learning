...
// decrypted code is [7+1+4, 1+4+5, 4+5+7, 5+7+1]. Notice that the numbers wrap around.
...
class Solution {
  ...
  public int[] decrypt(int[] code, int k) {
    ...
    // when k is +ve
    int s = 1; // start of the k sized window circularly
    int e = k; // end
    ...
    // now, we consider next windows and we slide
    for (int i = 0; i < n; i++) {
      ans[i] = sum;
      sum -= code[(s) % n]; // % n only for the case when `s` is at `n`
      sum += code[(e + 1) % n];
      s++;
      e++;
    }
  ...
  // ============================================
  // Brute Force
  public int[] decrypt_BRUTE(int[] code, int k) {
    int n = code.length;
    k = k % n;
