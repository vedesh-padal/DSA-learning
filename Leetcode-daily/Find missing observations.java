/*
  ...
  of the n + m rolls is exactly mean. If there are multiple valid answers, return any of them. 
...
// basically a distribution and Math problem
class Solution {
  public int[] missingRolls(int[] rolls, int mean, int n) {
      ...
      int missingSum = totalSum - obsSum;

      // impossible cases to have
      if (missingSum < n || missingSum > 6 * n)   {
          // so return empty array
          return new int[]{};
      }

      // for equal distribution among all
      int base = missingSum / n;
      int remainder = missingSum % n;

      int[] res = new int[n];
      // basically distributing the missing rolls to the ones within the range
      for (int i = 0; i < n; i++)
