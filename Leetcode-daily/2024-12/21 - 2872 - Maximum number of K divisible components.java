...
class Solution {
    ...
    private int dfs(int curr, int parent, List<Integer>[] graph, int[] values, int k, int[] compCnt) {
        // we are finding the subtree sum
        int sum = values[curr];

        // bottom up kind of sum building of each subtree
        for (int neighbor: graph[curr]) {
            if (neighbor != parent) {
                sum += dfs(neighbor, curr, graph, values, k, compCnt);
            }
        }

        // if this subtree / component has sum divisible by k, then we have a component
        if (sum % k == 0) {
            compCnt[0]++;

            // return 0;   // optional - will be an early exit
            // // indicating that we have found one, and we are cutting off this component
        }
    ...
    public int maxKDivisibleComponents(int n, int[][] edges, int[] values, int k) {
        ...
        // num of components, for the value to change, passing in an array
        int[] compCnt = new int[1]; // kind of pass by ref

        // can start from any node, here choosing from 0
        dfs(0, -1, graph, values, k, compCnt);
