...
class Solution {
  ...
  private long minCost(int roboPos, int factPos, List<Integer> robot, List<Integer> factories, long[][] dp) {

    // all robots are repaired
    if (roboPos < 0)
      return 0;

    // some robots are left but factories are over
    if (factPos < 0)
      return (long) (1e12);
    ...
    long pick = Math.abs(robot.get(roboPos) - factories.get(factPos))
        + minCost(roboPos - 1, factPos - 1, robot, factories, dp);

    long notPick = minCost(roboPos, factPos - 1, robot, factories, dp);

    return dp[roboPos][factPos] = Math.min(pick, notPick);
  }
