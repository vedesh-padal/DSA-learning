...
class Solution {
  ...
  private TreeNode findBST_WITH_ONE_UPPER_BOUND(int[] preorder, int[] nodeInd, int bound) {
    if (nodeInd[0] == preorder.length || preorder[nodeInd[0]] > bound) {
      return null;
    }

    TreeNode root = new TreeNode(preorder[nodeInd[0]++]);
    // we are going left, so bound will be current node's val
    root.left = findBST_WITH_ONE_UPPER_BOUND(preorder, nodeInd, root.val);
    
    // we are going right, so bound will be previous bound
    root.right = findBST_WITH_ONE_UPPER_BOUND(preorder, nodeInd, bound);
