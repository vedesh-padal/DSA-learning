 ...
 * function solve() which takes bt[] as input parameter and returns the average
...
class Solution {
  static int solve(int bt[] ) {
    // code here
    Arrays.sort(bt);
    int t = 0;  // to track the curr time
    int wtTime = 0;
    
    for (int i: bt) {
      wtTime += t;  // given that all processes arrived at t = 0
      t += i;
    }
